...
def plot_grid(images,N=10,C=10,figsize=(24., 28.),name='posterior'):
    ...
    plt.savefig('./xhat_'+name+'.pdf')
    plt.close()

'''
self-coded helper function to save pictures
'''
## calls the plot_grid function and saves the file with a descriptive name
#  @param sample: sampled values from prior or posterior
#  @param type: type of teh data - 'color' or 'bw'
#  @param epochs: number of epochs used to train the model that provided the output
#  @param distr: 'posterior' or 'prior'
def save_picture(sample, data_type, epochs, distr):
    #save file with descriptive name
    data_type = "color" if "color" == data_type else "bw"
    filename = f"{distr}_{data_type}_{epochs}epochs"
    plot_grid(sample, name=filename)
    print(f'Saved: ./xhat_{filename}.pdf')
